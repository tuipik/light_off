...
1. Разовий старт у headful через `Xvfb` усередині контейнера:
```bash
...
- Якщо Incapsula блокує, запустіть контейнер разово з `PLAYWRIGHT_HEADLESS=0` — в Docker `start.sh` підіймає `Xvfb`, тому окремий дисплей не потрібен. Після проходження перевірки поверніть `PLAYWRIGHT_HEADLESS=1`.
